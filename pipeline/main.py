...
def run(video_path: Path, work_dir: Path, db_dir: Path, output_dir: Path,
        dry_run: bool, no_demucs: bool) -> int:
    log(f"video: {video_path}")
    log("step 1/6: preprocess audio")
    audio_path = prepare_audio(video_path, work_dir, do_demucs=not no_demucs)
    ...
    if cache.exists():
        ...
        embs, times = extract_embeddings(audio_path)
...
def main(argv=None) -> int:
    ...
    p.add_argument("--no-demucs", action="store_true",
                   help="Skip Demucs vocal separation (faster but less accurate with BGM).")
    args = p.parse_args(argv)

    args.work_dir.mkdir(parents=True, exist_ok=True)
    return run(args.video, args.work_dir, args.db_dir, args.output_dir,
               args.dry_run, args.no_demucs)
